			...
			if ($publish_id_count == 0) {
				...
				if ($sermon_title_count>0){ 
					...
					if (isset($_FILES['reg_thumbnail']) && $_FILES['reg_thumbnail']['error'] != UPLOAD_ERR_NO_FILE) {
						...
						mysqli_query($conn,"UPDATE publish_tab SET reg_title='$reg_title', status_id='$status_id', modified_by='$login_staff_id', `updated_time`=NOW(), blog_cat_id='$blog_cat_id' WHERE publish_id='$publish_id'") or die (mysqli_error($conn));
