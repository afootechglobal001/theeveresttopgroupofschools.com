	...
	if($check==0){ 
		...
		if (empty($status_id)){
			...
			if ($faq_quest_count>0){ 
				...
				(`page_category_id`, `publish_id`, `faq_question`, `faq_answer`, `status_id`, `modified_by`, `created_time`, `updated_time`, `faq_cat_id`) VALUES  
				('$page_category_id', '$publish_id', '$faq_question', '$faq_answer', '$status_id', '$login_staff_id', NOW(), NOW(), '$faq_cat_id')")or die (mysqli_error($conn));
